...
void KeyExchangeManager::exchangeTlsKeys(const std::string& type, const SeqNum& bft_sn) {
  ...
  const std::string base_path = bftEngine::ReplicaConfig::instance().certificatesRootPath;
  std::string root_path = (use_unified_certs) ? base_path + "/" + std::to_string(repID_)
                                              : base_path + "/" + std::to_string(repID_) + "/" + type;
