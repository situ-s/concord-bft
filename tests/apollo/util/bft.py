...
def with_bft_network(start_replica_cmd, selected_configs=None, num_clients=None, num_ro_replicas=0,
                     ...
                     break_on_first_success=False, unified_cert=False):
    """
                    ...
                    with test_instance.subTest(config=f'{bft_config}'):
                        async with trio.open_nursery() as background_nursery:
                            ...
                            async def test_with_bft_network():
                                with BftTestNetwork.new(config, background_nursery, with_cre=with_cre, unified_cert=unified_cert) as bft_network:
...
class BftTestNetwork:
    ...
    @classmethod
    def new(cls, config, background_nursery, client_factory=None, with_cre=False, unified_cert=False):
        ...
        bft_network.unified_cert = unified_cert
        ...
        if bft_network.comm_type() == bft_config.COMM_TYPE_TCP_TLS:
            ...
            bft_network.generate_tls_certs(
                bft_network.num_total_replicas() + config.num_clients + RESERVED_CLIENTS_QUOTA + generate_cre, unified_cert=unified_cert)
    ...
    def start_cre(self):
        ...
        with log.start_action(action_type="start_cre"):
            ...
            if os.environ.get('KEEP_APOLLO_LOGS', "").lower() in ["true", "on"]:
                ...
                print("SS-- cre unified cert value", self.unified_cert)
                cre_cmd = [cre_exe,
                           "-i", str(self.cre_id),
                           "-f", str(self.config.f),
                           "-c", str(self.config.c),
    ...
    async def change_configuration(self, config, generate_tls=False, unified_cert=False):
        self.config = config
        ...
        if generate_tls and self.comm_type() == bft_config.COMM_TYPE_TCP_TLS:
            ...
            self.generate_tls_certs(self.num_total_replicas() + config.num_clients + RESERVED_CLIENTS_QUOTA + generate_cre, unified_cert=unified_cert)
    ...
    def generate_tls_certs(self, num_to_generate, start_index=0, unified_cert=False):
        """
        ...
        args = [certs_gen_script_path, str(num_to_generate), temp_cert_dir, str(start_index), str(int(unified_cert))]
        print("SS---", args)
