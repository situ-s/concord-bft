...
class TcpTlsClient(BftClient):
    ...
    def _get_private_key_path(self, replica_id, *, is_client):
        ...
        """
        if self.config.unified_certs :
            print("HI--ss client unified cert pk")
    ...
    async def _establish_ssl_stream(self, dest_replica, lot):
        ...
        print("SS-- server path", server_cert_path)
        print("SS-- client path", client_cert_path)
        print("SS-- pk path", client_pk_path)
        ...
        print("SS-- server name", server_hostname)
